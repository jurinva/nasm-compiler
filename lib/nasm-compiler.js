...
export default {
  ...
  createToolbar() {
    ...
    // Измененный способ добавления панели
    const statusBar = document.querySelector('status-bar');
    if (statusBar) {
      statusBar.insertBefore(this.toolbar, statusBar.firstChild);
    } else {
      atom.notifications.addWarning('Could not find status bar', {
        detail: 'NASM Compiler toolbar will not be displayed'
      });
    }
